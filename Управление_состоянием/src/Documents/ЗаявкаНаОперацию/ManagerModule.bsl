Функция Модель(Контекст, Знач ПутьКДанным = "") Экспорт
	...
	РаботаСМодельюКлиентСервер.Связь(Контекст, Модель, "ЭтоСчетОрганизации", "ТипОперацииБюджетирование", "ТипОперации");
	РаботаСМодельюКлиентСервер.Связь(Контекст, Модель, "ЭтоСчетКонтрагента", "ЭтоСчетОрганизации", "ЭтоСчетОрганизации");

	РаботаСМодельюКлиентСервер.Связь(Контекст, Модель, "СчетКонтрагента", "Организация", "Отбор.Владелец").Свойства.Вставить("Использование", "ЭтоСчетОрганизации");
	РаботаСМодельюКлиентСервер.Связь(Контекст, Модель, "СчетКонтрагента", "Контрагент", "Отбор.Владелец").Свойства.Вставить("Использование", "ЭтоСчетКонтрагента");
	...
	//  Зависимые связи
	Модель.ЗависимыеСвязи["ЭтоСчетОрганизации"] = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("СчетКонтрагента-Организация");
	Модель.ЗависимыеСвязи["ЭтоСчетКонтрагента"] = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("СчетКонтрагента-Контрагент");
	
	//  Настройка параметров состояния объекта
	Модель.Параметры["ЭтоСчетОрганизации"].ВыражениеЗначения = "Параметры.ТипОперации = ПредопределенноеЗначение(""Перечисление.ТипыОперацийБюжетирование.ПеремещениеМеждуМестамиХраненияСредств"")
	| ИЛИ Параметры.ТипОперации = ПредопределенноеЗначение(""Перечисление.ТипыОперацийБюжетирование.КонвертацияВалюты"")";
	Модель.Параметры["ЭтоСчетОрганизации"].НаСервере = Ложь;
	Модель.Параметры["ЭтоСчетКонтрагента"].ВыражениеЗначения = "НЕ Параметры.ЭтоСчетОрганизации";
	Модель.Параметры["ЭтоСчетКонтрагента"].НаСервере = Ложь;
