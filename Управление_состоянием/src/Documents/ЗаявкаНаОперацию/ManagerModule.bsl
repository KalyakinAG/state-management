Функция Модель(Контекст, Знач ПутьКДанным = "") Экспорт
	...
	Модель.Параметры["ЭтоВнутренняяОперация"].ВыражениеЗначения = "Параметры.ТипОперации = ПредопределенноеЗначение(""Перечисление.ТипыОперацийБюжетирование.ПеремещениеМеждуМестамиХраненияСредств"")
	...
	Модель.Параметры["ЭтоВнешняяОперация"].ВыражениеЗначения = "НЕ Параметры.ЭтоВнутренняяОперация";
