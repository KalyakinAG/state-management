	...
	Для Каждого ИДСвязи Из Параметр.ВходящиеСвязи Цикл
		...
		Если ЗначениеЗаполнено(Связь.Источник) Тогда
			...
			Если ПараметрИсточника.ПроверкаЗаполнения И НЕ ЗначениеЗаполнено(ЗначениеПараметра) Тогда
				...
				Иначе
					Поле = ?(ПараметрИсточника.ЭтоДанныеКонтекста, "", Модель.ПутьКДанным + ".") + ПараметрИсточника.Идентификатор;
				КонецЕсли;
...
Процедура УправлениеФормой(Контекст, Модель, ИзмененныеРеквизиты = Неопределено, ЗависимыеЭлементы = Неопределено, НаСервере = Ложь) Экспорт
...
Процедура ПриАктивизацииСтроки(Форма, Элемент) Экспорт
	...
	Иначе
		...
		КонецЕсли; 
		РаботаСМодельюКлиентСервер.УстановитьЗначениеПараметра(Форма, Модель, Модель.Параметры[ПараметрКоллекции.Идентификатор + ".ИндексСтроки"],, ИндексСтроки);
	КонецЕсли;
КонецПроцедуры
