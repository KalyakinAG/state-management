...
Функция УстановитьЗначение(Контекст, _Параметр, ИндексСтроки = Неопределено, Значение, ИзмененныеРеквизиты = Неопределено) Экспорт
	Перем ИмяРеквизита;
...
//
Функция ЗаполнитьЗначение(Контекст, Объект, Модель, СсылкаНаЗначение, Параметр, ВходящиеПараметры, ДанныеСтроки = Неопределено, ИндексСтроки = Неопределено) Экспорт
	ЕстьИзменение = Ложь;
		...
		Для Каждого Реквизит Из ИзмененныеРеквизиты Цикл
			...
			Если Параметр.ЭтоЭлементКоллекции Тогда
				...
				Если НЕ ЗначениеЗаполнено(СсылкаНаЗначение) Тогда
					Продолжить;
				КонецЕсли;
				Если РаботаСМодельюКлиентСервер.ЗаполнитьЗначение(Контекст, Объект, Модель, 
						СсылкаНаЗначение, 
						Параметр,
						ВходящиеПараметры,
						ДанныеСтроки,
