...
//
Процедура РассчитатьСостояние(Контекст, Модель, ИзмененныеРеквизиты, РассчитанныеРеквизиты, Последовательность, ИспользованныеСвязи, НаСервере = Ложь) Экспорт
	Перем ИмяРеквизита;// данная переменная определяется в функции ПолучитьРеквизитФормыПоПути
	Если ЗначениеЗаполнено(Модель.ПутьКДанным) Тогда
	...
	Пока Истина Цикл
		...
		Если РассчитанныеРеквизиты[Реквизит.Идентификатор] = Неопределено Тогда
			...
			Иначе
				//  Это чистый параметр
				ЕстьИзменение = ЗаполнитьЗначение(Контекст, Модель, ПолучитьОбъектПараметра(Контекст, Модель, Параметр,, ИмяРеквизита)[ИмяРеквизита], Параметр, ВходящиеПараметры);
			КонецЕсли;
