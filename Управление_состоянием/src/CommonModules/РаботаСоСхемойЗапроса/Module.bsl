...
Функция ЗапросПакетаИзТаблицы(СхемаЗапроса, Знач ТаблицаИсточника, Знач ПсевдонимИсточника = "", Знач ВыбираемыеКолонки = Неопределено, ТаблицаДляПомещения = "", ЗапросНаУничтожение = Ложь, ВыбиратьРазрешенные = Ложь, ОператорВыбрать = Неопределено, Очистить = Ложь) Экспорт
	...
	Если ВыбираемыеКолонки <> Неопределено Тогда
		...
		Если ТипВыбираемыеКолонки = Тип("Строка") ИЛИ ТипВыбираемыеКолонки = Тип("Массив") Тогда
			Для каждого Колонка Из ОбщийКлиентСервер.Массив(ВыбираемыеКолонки) Цикл
				Поле(ЗапросПакета, ОператорВыбрать,, Колонка);
			КонецЦикла;
		Иначе
			...
			Для каждого Колонка Из ВыбираемыеКолонки Цикл
				Если ДоступныеПоля.Найти(Колонка.Имя) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				Поле(ЗапросПакета, ОператорВыбрать,, Колонка.Имя);
			КонецЦикла;
		КонецЕсли;
	...
	Иначе
		Для Индекс = 0 По МассивПараметров.ВГраница() Цикл
			...
			Если ПустаяСтрока(Выражение) Тогда
				Продолжить;
			КонецЕсли;
			СтруктураПараметров.Вставить(МассивПараметров[Индекс], ОбщийКлиентСервер.ПолучитьЭкранированныйТекст(Выражение));
		КонецЦикла;
	КонецЕсли;
	Возврат ?(СтруктураПараметров.Количество() = 0, Неопределено, СтруктураПараметров);
КонецФункции

Функция ДоступныеПоля(Колонки, ДоступныеКолонки = Неопределено) 
	ДоступныеПоля = Новый Массив;
	Для каждого Колонка Из Колонки Цикл
		Если ДоступныеКолонки <> Неопределено И ДоступныеКолонки.Найти(Колонка.Имя) <> Неопределено Тогда
			Продолжить;
		КонецЕсли; 
...
// Возвращает структуру с описанием источника
//
// Параметры:
...
//
Функция ОписаниеВременнойТаблицы(ИмяТаблицы, Источник = Неопределено, КоллекцияКолонок = Неопределено) Экспорт
	...
	Если ТипЗначенияИсточника = Тип("МенеджерВременныхТаблиц") Тогда
		#Если Сервер И НЕ Сервер Тогда
			Источник = Новый МенеджерВременныхТаблиц;
		#КонецЕсли
		Если Источник.Таблицы.Найти(ИмяТаблицы) = Неопределено Тогда
			Возврат ИмяТаблицы;
		КонецЕсли;
		ДоступныеПоля = ДоступныеПоля(Источник.Таблицы[ИмяТаблицы].Колонки, мКолонки);
	ИначеЕсли ТипЗначенияИсточника = Тип("ТаблицаЗначений") Тогда
		ДоступныеПоля = ДоступныеПоля(Источник.Колонки, мКолонки);
	ИначеЕсли ЭтоТабличнаяЧасть(ТипЗначенияИсточника) ИЛИ ТипЗначенияИсточника = Тип("ДанныеФормыКоллекция") Тогда
		ДоступныеПоля = ДоступныеПоля(Источник.Выгрузить(Новый Массив).Колонки, мКолонки);
		...
		Если ПозицияТочки = 0 Тогда // поля по всем источникам, доступным из оператора
			Для каждого Источник Из Оператор.Источники Цикл
				...
				Для каждого ДоступноеПоле Из Источник.Источник.ДоступныеПоля Цикл
					...
					Если ВыбираемыеПоля.Найти(мПутьКПолю) <> Неопределено Тогда
						Продолжить;
					КонецЕсли;
...
Процедура ДобавитьУсловиеСоединения(Соединение, Условие)
	...
	Если ПустаяСтрока(УсловиеСоединения) Тогда
		УсловиеСоединения = Условие;
	Иначе
		УсловиеСоединения = УсловиеСоединения + " И ("+Условие+")";
	КонецЕсли;
...
//
Функция Запрос(СхемаЗапроса, Параметры = Неопределено, МенеджерВременныхТаблиц = Неопределено, СоздатьМенеджерВременныхТаблиц = Ложь, ОчиститьСхему = Истина) Экспорт
	Запрос = Новый Запрос;
	Если СоздатьМенеджерВременныхТаблиц Тогда
...
#Область ПоддержкаСКД
	
Процедура СкопироватьЭлементыСхемыЗапроса(ПриемникЗначения, ИсточникЗначения, ОчищатьПриемник = Истина) Экспорт
	Если ОчищатьПриемник Тогда
		ПриемникЗначения.Очистить();
	КонецЕсли;
    Для каждого ЭлементИсточник Из ИсточникЗначения Цикл
        ПриемникЗначения.Добавить(ЭлементИсточник);
	КонецЦикла;
КонецПроцедуры

// Конструирует схему запроса по описанию из схемы компоновки данных
//
// Параметры:
//	СхемаКомпоновкиДанных 		- СхемаКомпоновкиДанных
//	ВариантНастроек       		- Строка - название варианта настроек
//	НастройкиКомпоновкиИлиОтбор - НастройкиКомпоновкиДанных - 
//								- ОтборКомпоновкиДанных
//  ПараметрыЗапроса 			- Структура - формируется из параметров отбора СКД
//
// Возвращаемое значение:
//	СхемаЗапроса
//
Функция СхемаЗапросаКомпоновкиДанных(СхемаКомпоновкиДанных, ВариантНастроек = "", НастройкиКомпоновкиИлиОтбор = Неопределено, ПараметрыЗапроса = Неопределено) Экспорт 
	...
	Если ЗначениеЗаполнено(ВариантНастроек) Тогда
		КомпоновщикНастроек.ЗагрузитьНастройки(СхемаКомпоновкиДанных.ВариантыНастроек[ВариантНастроек].Настройки);
	Иначе
		КомпоновщикНастроек.ЗагрузитьНастройки(СхемаКомпоновкиДанных.НастройкиПоУмолчанию);
	КонецЕсли;
	
	ТипЗначенияНастройки = ТипЗнч(НастройкиКомпоновкиИлиОтбор);
	Если ТипЗначенияНастройки = Тип("НастройкиКомпоновкиДанных") Тогда
		КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиКомпоновкиИлиОтбор);
	ИначеЕсли ТипЗначенияНастройки = Тип("ОтборКомпоновкиДанных") Тогда
		КомпоновкаДанных.СкопироватьЭлементыНастройкиСКД(КомпоновщикНастроек.Настройки.Отбор, НастройкиКомпоновкиИлиОтбор);
	КонецЕсли;
	...
	Для Каждого ЗначениеПараметра Из МакетКомпоновкиДанных.ЗначенияПараметров Цикл
		Если RegExp.Test("&"+ЗначениеПараметра.Имя) Тогда
			ИмяПараметра = "П"+ОбщийКлиентСервер.ИмяПоУникальномуИдентификатору();
		Иначе
			ИмяПараметра = ЗначениеПараметра.Имя;
		КонецЕсли;
		СоответствиеПараметров.Вставить(ЗначениеПараметра.Имя, ИмяПараметра);
	КонецЦикла;
	
	Отбор = Новый Массив;
	Для каждого ВыражениеОтбора Из ОператорВыбрать.Отбор Цикл
		...
		Для каждого Match Из Matches Цикл
			...
			ИмяПараметра		= Прав(Match.Value, СтрДлина(Match.Value)-1);
			НовоеИмяПараметра	= СоответствиеПараметров[ИмяПараметра];
			ТекстВыражения		= ТекстВыражения + ДельтаТекста + "&" + НовоеИмяПараметра;
			ПараметрыЗапроса.Вставить(НовоеИмяПараметра, МакетКомпоновкиДанных.ЗначенияПараметров[ИмяПараметра].Значение);
		КонецЦикла;
		...
		Отбор.Добавить(ТекстВыражения);
	КонецЦикла;
	СкопироватьЭлементыСхемыЗапроса(ОператорВыбрать.Отбор, Отбор);
	Возврат СхемаЗапроса;
КонецФункции

// Добавляет условия отбора из элементов СКД в отбор оператора схемы запроса
//
// Параметры:
//	ОператорВыбрать 		- ОператорСхемыЗапроса
//	СхемаКомпоновкиДанных	- СхемаКомпоновкиДанных 
//  ОтборКомпоновкиДанных   - ОтборКомпоновкиДанных
//	ПараметрыЗапроса        - Структура - формируется из параметров отбора СКД
//	ОчиститьПриемник        - Булево - признак очистки отбора оператора перед добавлением
//
Процедура СкопироватьОтборКомпоновкиДанных(ОператорВыбрать, СхемаКомпоновкиДанных, ОтборКомпоновкиДанных, ПараметрыЗапроса, ОчиститьПриемник = Ложь) Экспорт 
	КомпоновщикНастроек = Новый КомпоновщикНастроекКомпоновкиДанных;
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКомпоновкиДанных));
	КомпоновщикНастроек.ЗагрузитьНастройки(СхемаКомпоновкиДанных.НастройкиПоУмолчанию);
	КомпоновкаДанных.СкопироватьЭлементыНастройкиСКД(КомпоновщикНастроек.Настройки.Отбор, ОтборКомпоновкиДанных);
	...
	Отбор = Новый Массив;
	Для каждого ВыражениеОтбора Из ЗапросПакета.Операторы[ЗапросПакета.Операторы.Количество()-1].Отбор Цикл
		ТекстВыраженияОтбора	= Строка(ВыражениеОтбора);
		ТекстВыражения			= "";
		ИндексПозиции			= 1;
		Matches					= RegExp.Execute(ТекстВыраженияОтбора);
		...
		Отбор.Добавить(ТекстВыражения);
	КонецЦикла;
	СкопироватьЭлементыСхемыЗапроса(ОператорВыбрать.Отбор, Отбор, ОчиститьПриемник);
КонецПроцедуры
