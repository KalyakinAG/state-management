...
Процедура ОбновитьДанные(Контекст, ИзмененныеРеквизиты) Экспорт
	Перем ИмяРеквизита;
	...
	Для Каждого ЭлементСписка Из Список Цикл
		...
		Если Параметр.ЭтоЭлементКоллекции Тогда
			...
			Для Каждого ЭлементКоллекции Из Коллекция Цикл
				ИндексСтроки = Коллекция.Индекс(ЭлементКоллекции);
				...
				Отказ = Ложь;
				ВходящиеПараметры = РаботаСМодельюКлиентСервер.ПараметрыСвязей(Контекст, Модель, ПараметрКоллекции,,, Отказ);
				Если Отказ Тогда
					Продолжить;
				КонецЕсли;
