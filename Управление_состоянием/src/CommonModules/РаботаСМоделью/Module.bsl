...
// 
Процедура ОбновитьДанные(Контекст, ИзмененныеРеквизиты = Неопределено, ТихийРежим = Ложь) Экспорт
	Перем ИмяРеквизита;
	//Сообщить("ОбновитьДанные");
	...
	Для Каждого ЭлементСписка Из Список Цикл
		...
		Иначе
			...
			Если Отказ Тогда
				Если НЕ Параметр.СохраняемыеДанные И ОбщийКлиентСервер.УстановитьЗначение(СсылкаНаЗначение, Параметр.ЗначениеПоУмолчанию) Тогда
					Сообщить(СтрШаблон("Значение изменено %1: %2 -> %3", Параметр.Идентификатор, ОбъектЗначения[ИмяРеквизита], СсылкаНаЗначение));
				...
				Продолжить;
			КонецЕсли;
			Если Параметр.ЭтоСсылка И НЕ ЗначениеЗаполнено(СсылкаНаЗначение) И НЕ Параметр.ПроверкаЗаполнения Тогда
				Продолжить;
			КонецЕсли;
			Если РаботаСМодельюКлиентСервер.ОпределитьЗначение(Контекст, Объект, Модель, 
				СсылкаНаЗначение, 
